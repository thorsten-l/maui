...
public class AppController
{
  ...
  @GetMapping("/debug-oidc")
  public String debugOIDC(Model model,
    @AuthenticationPrincipal DefaultOidcUser principal)
  {
    LOGGER.debug("debugOIDC");
    
    model.addAttribute("fullname", principal.getFullName());
    model.addAttribute("issuer", principal.getIssuer().toExternalForm());
    model.addAttribute("principal", principal);
    model.addAttribute("idToken", principal.getIdToken());
    model.addAttribute("accessToken", mauiApiClientService.getAccessToken());
